...
public class CustomClassUtils {
    ...
    public static void declaredFields(Class clazz, Map<Integer, Field> sortedAllFiledMap,
                                      ...
                                      Boolean needIgnore, Holder holder, int whichAnnotationPropertyUsed) {
        FieldCache fieldCache = getFieldCache(clazz, convertAllFiled, whichAnnotationPropertyUsed);
        if (fieldCache == null) {
            return;
        }
        if (ignoreMap != null) {
    ...
    public static void declaredFields(Class clazz, Map<Integer, Field> sortedAllFiledMap, Boolean convertAllFiled,
                                      Boolean needIgnore, WriteHolder writeHolder, int whichAnnotationPropertyUsed) {
        declaredFields(clazz, sortedAllFiledMap, null, null, convertAllFiled, needIgnore, writeHolder, whichAnnotationPropertyUsed);
    }

    private static FieldCache getFieldCache(Class clazz, Boolean convertAllFiled, int whichAnnotationPropertyUsed) {
        if (clazz == null) {
            return null;
        }
        ...
        synchronized (clazz) {
            ...
            declaredFields(clazz, convertAllFiled, whichAnnotationPropertyUsed);
        }
        return FIELD_CACHE.get(clazz).get();
    }

    private static void declaredFields(Class clazz, Boolean convertAllFiled, int whichAnnotationPropertyUsed) {
        ...
        for (Field field : tempFieldList) {
            declaredOneField(field, orderFiledMap, indexFiledMap, ignoreMap, excelIgnoreUnannotated, convertAllFiled, whichAnnotationPropertyUsed);
        }
    ...
    private static void declaredOneField(Field field, Map<Integer, List<Field>> orderFiledMap,
                                         ...
                                         Boolean convertAllFiled, int whichAnnotationPropertyUsed) {
        ...
        // 使用index
        if (excelProperty != null && excelProperty.index() >= 0 && whichAnnotationPropertyUsed == 0) {
